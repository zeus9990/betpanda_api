...
// Constant-time-ish string comparison to avoid trivial timing attacks on the API key.
...
function isAuthorized(request, env) {
  ...
  return Boolean(env.API_KEY) && Boolean(providedKey) && safeEqual(providedKey, env.API_KEY);
}
...
async function handleStats(request, env) {
  try {
    if (!isAuthorized(request, env)) {
      return new Response(JSON.stringify({ error: "Unauthorized. Missing or invalid x-api-key header." }), {
        status: 401,
